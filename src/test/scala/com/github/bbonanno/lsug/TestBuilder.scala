...
trait TestBuilder {
  ...
  implicit def EventEq[E <: Event]: Equality[E] =
    (a: E, b: Any) =>
      (a, b) match {
        case (OrderStatusEvent(_, _, s1), OrderStatusEvent(_, _, s2)) => s1 == s2
        case _                                                        => false
    }
  ...
  implicit val MyPrettifier: Prettifier = Prettifier {
    ...
    case OrderStatusEvent(id, timestamp, status) => s"OrderStatusEvent(id=$id, timestamp=$timestamp, status=${status.prettify})"
